...
echo "  sudo aws s3 cp s3://your-s3-bucket/cloudscale-devtools.zip /tmp/plugin.zip && sudo rm -rf /var/www/html/wp-content/plugins/cloudscale-devtools && sudo unzip -q /tmp/plugin.zip -d /var/www/html/wp-content/plugins/ && sudo chown -R apache:apache /var/www/html/wp-content/plugins/cloudscale-devtools && php -r \"if(function_exists('opcache_reset'))opcache_reset();\""
